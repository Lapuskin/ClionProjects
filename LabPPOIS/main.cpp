...
Long operator + (Long l1, Long l2) {
    ...
    if (l2._isNegative)
        return l1 - (-l2);
    if (l1._value.size() < l2._value.size())
        for (int i = 0; i < l2._value.size() - l1._value.size() + 1; i++)
            l1._value.push_back(0);
    else
    if(l1._value.size() > l2._value.size())
        for (int i = 0; i < l1._value.size() - l2._value.size() + 1; i++)
...
Long division(Long l1, Long l2, int i, Long temp, Long res){
    while (temp < l2) { /// высчитываем число, скоторым будут проводиться первые операции
        temp = temp * 10;
        if(i >= 0) {
